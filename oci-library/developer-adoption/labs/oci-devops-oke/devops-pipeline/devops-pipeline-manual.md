...
1. Let's open the OCI DevOps project that was created by the SRE team through ORM. Go to OCI Navigation Menu -> Developer Services -> DevOps -> Projects. The project name should starts with `hellow_`
...
1. Fill ou the form with the following data:
    ...
    |--|--|
    |Name|carts-kubernetes-deploy-dev.yml|
    |Type|Kubernetes Manifest|
    |Artifact Source|Inline|
    |Value|Paste the content of the file `kubernetes-deploy.yml`. Make sure you copied the raw file.|
    ...
    ```
        ...
        containers:
        - name: carts
            image: ${mushop_carts_container_image}
            ports:
            - containerPort: 80

    ```
    Now, you define the version of the image during the pipeline.
    ![devops artifact](./images/devops-artifact.png) 

## **STEP 3**: Create DevOps Pipeline

Let's create now a DevOps pipeline for publishing the mushop carts service. This is going to publish the artifact to our OKE environment. 
...
1. Enter a name and a description for the pipeline and then click on Create pipeline:
    |Property Name|Property Value|
    |--|--|
    |Name|deploy-mushop-carts-to-dev|
    |Description|Deploy Mushop Carts service to OKE Development environment|

    
1. Add a Stage to your Pipeline. Click on the plus icon to add a Stage.
    ![Create devops pipeline](./images/create-devops-pipeline.png) 

1. In our use case, we are applying the Kubernetes manifest to our Development environment in OKE. If we needed to enforce more security controls, we could add a stage requesting someone to approve the stage in order to proceed to the next one. In our case, let's simply select `Apply manifest to your Kubernetes cluster`. Then click on next.

    ![k8s manifest](./images/devops-add-stage.png) 

1. In the next window, enter the following data then click on Add:

    |Property Name|Property Value|
    |--|--|
    |Stage Name|deploy-carts-service|
    |Description|Deploy Carts service to Dev|
    |Environment|test_oke_env|
    |Select one or more artifacts|carts-kubernetes-deploy-dev.yml|
    |If validation fails, automatically rollback to the last successful version?|Yes|

    ![devops stage](./images/devops-stage-carts.png) 

1. Here is the pipeline we created.
![devops pipeline deploy carts](./images/devops-pipeline-deploy-carts.png) 

## **STEP 4**: Deploy Carts Service to OKE

You can run a pipeline directly from the OCI Console or you can build integrations with the API, CLI or some external integrations. But before running the pipeline, we need to set a value for the parameter we used in the Artifact. 

1. Click on Parameter tab and enter the following data:

    |Name|Default Value|Description|
    |--|--|--|
    |mushop_carts_container_image|Use the URL of the image published on the OCIR in the format of `<region-key>.ocir.io/<tenancy-namespace>/<repository>:<tag>`|Container Image for Mushop Carts|

1. Go back to Pipeline tab and click on `Run Pipeline` button on top right corner of the page. 
