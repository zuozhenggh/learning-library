...
3. Now we configure how to train our model.
   ...
   Data Source: F1.RACES_TRAIN (choose F1 and RACES_TRAIN on the popup window). Remember that this table holds the races until the end of 2019.
...
8. The training will take several minutes. During this time, AutoML tries out several different ML algorithms, with different configurations.
   ...
   The value under Negative Mean Absolute Error is an indicator of the accuracy of the model. The exact values and order of the models can be different on your environment due to randomness in the process.
...
1. Calculate the distance between the predicted and the actual scores.
   ...
   Create the formula ABS(Prediction-SCORE). Drag the names of the fields over to the calculation canvas.
