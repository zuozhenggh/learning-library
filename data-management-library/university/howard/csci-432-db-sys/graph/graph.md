...
- The following task requires an Autonomous Database - Shared Infrastructure account. 
...
5.  Now let's change the default Vertex and Edge labels.  
    ...
    This is **important** because we will use these edge labels in the next task of this lab when querying the graph.  
...
8. Click **Next** and then click **Create Graph** to move on to the next step in the flow.   
   ...
   That graph name is used throughout the next task.  
   Do not enter a different name because then the queries and code snippets in the next task will fail.  
...
## Task 5: Graph Studio: Query and visualize a graph using PGQL paragraphs in a notebook
...
- Earlier tasks of this lab. That is, the graph user exists and you have logged into Graph Studio. 

**Verify that `BANK_GRAPH` is loaded into memory**
...
**Alternate and optional step: Import the notebook**
...
5. Once imported it should open in Graph Studio.  
   ...
   You can execute the paragraphs in sequence and change visualizations settings as described in **Query the `BANK_GRAPH` and visualize the results** below.  
   
**Create a notebook (Not necessary if you imported the notebook)**
 ...
 **Query the `BANK_GRAPH` and visualize the results**
