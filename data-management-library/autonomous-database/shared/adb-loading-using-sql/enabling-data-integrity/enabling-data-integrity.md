...
3. This time, the data load process will fail with the following error message in the script output window that indicates the duplicate records have been rejected: (ORA-20000: ORA-00001: unique constraint (ADMIN.SYS_C0022901) violated...):
    ...
    **NOTE:** The data integrity checking does not depend on the name of the source data file. You could use a different filename (i.e. a filename that has not been used before) and you would get the same result - an error message that indicating the data file contains records that have already been loaded.
