...
2. Prepare some acceleration and velocity attributes to try to understand customer behavior
  ...
  We notice that the mechanics work, so next we will create a View on top of the entire `CUSTSALES` to continue with the creation of the Acceleration and Velocity attributes.
  ...
  You will note that there are plain Averages being calculated, but also we are computing **RATIOS**.  An example of the calculation of Averages in the code is below.

  ![Churn Notebook Step 2 Pivot Additional Attributes](images/oml-churn-note-step2-pivot-add-attr.png "Churn Notebook Step 2 Pivot Additional Attributes")

  In the case of the RATIO calculations, it is important to make sure the denominator of the division is not "0", so we use a CASE statement to verify and avoid this condition, like the below.
  ...
  Let's take a look at the data on our TRENDS_SALES_TRANS view and get a quick preview of it.  Remember to scroll to the right to check most of the new attributes created.
...
3. Prepare a pivotting table for several of the transaction features
  ...
  We will now explore the different categorical columns that we can use for pivotting.  
  ...
  ![Churn Notebook Step 3 check Movies](images/oml-churn-note-step3-check-movies.png "Churn Notebook Step 3 check Movies")

  We will now create a Pivotting Map for all the different Categorical attributes we have available in the `CUSTSALES` table, so that we can understand how many movies the customers watched in the different Devices or Browser types, what type of payment methods they prefer, different times they watched the Popular movies or how many movies of each Genre they like.  This will give us a good insight into customer behavior.
  ...
  ![Churn Notebook Step 3 Final check](images/oml-churn-note-step3-final-check.png "Churn Notebook Step 3 Final check")
...
4. Prepare a column that will represent whether a customer has churned (1) or not(0) in the last available month
  ...
  In that sense, we will create a query that will have a new attribute called **TARGET** that will be defined as:  
  ...
  Let's check the number of customers that would enter this definitions, and also check the number of churners.  We expect a proportion of TARGET=1 (churners) of around 1.15% on this example.
...
5. Build the final dataset for Machine Learning
  ...
  - The `TRENDS_SALES_TRANS` pivoted view and the velocity and acceleration attributes
  ...
  The beginnig of the query is here: 
...
In this task, you will use the Oracle Machine Learning (OML) AutoML UI provided with your Autonomous Data Warehouse and its features to identify customers with a higher likelihood of churning from **Oracle MovieStream** streaming services to a different movie streaming company.

We have prepared the final table called `MOVIESTREAM_CHURN` in the previous Tasks, so now we can use it in our Analysis.

1. Open the OML AutoML UI interface from the Main OML Notebooks menu and create an **Experiment**.
  ...
  You should see the OML AutoML UI Experiments menu, which should be empty.  Let's create a **New Experiment** by clicking on the button indicated below.
  ...
  Then, click on the **Loupe Icon** at the right of the **Data Source** field, so that we can find the Table `MOVIESTREAM_CHURN` we have just created.
  ...
  ![Churn AutoML Step 1 select data source](images/oml-churn-automl-select-data.png "Churn AutoML Step 1 select data source")
 
  Back in the **Experiment** page, the **Data Source** name is populated with our selection, and OML AutoML UI will automatically compute several statistics for each attribute.
...
2. Define the Prediction Target, Case ID and Experiment Settings
  ...
  On the left hand side, just under the **Data Source**, you should have noted that now the **Prediction Type** pull-down option is showing **Classification**.  This is because OML AutoML UI detects that our recently selected **Predict**, our `TARGET` attribute, has only two different values and as such this is a binary classification problem.  The other option is **Regression**, which is not appropriate for our problem, so we just leave it as is.
  ...
  We are going to leave the **Algorithms** selections as is, which means that OML AutoML UI will try all of those different types of algorithms to find the best one for us.
  
  Here's how our selections look like.
...
3. Run the Experiment and explore the results
  ...
  By clicking on the **three dots** themselves, a full floating **Progress** indicator opens up, showing the specific step of the process the **Experiment** is currently running.

  ![Churn AutoML Step 3 open progress](images/oml-churn-automl-open-progress.png "Churn AutoML Step 3 open progress")
  ...
  Please Note: this Importance is **not** related to any of the models, but it shows an overall value to give us an idea of potentially which attributes would be important.
  ...
  It can be seen that in this **Experiment**, the attributes with the highest **Overall Importante** are:
    ```
  ...
  Scrolling up to the **Leader Board** section, we can click on the **Random Forest** unique model name to open the diagnostics.  Click on the Random Forest **Model Name** link in blue (it would be unique to you).  In the following example it is called *rf_d4d2503940*.
  ...
  The first screen that shows up is the **Prediction Impacts**.  This time the attributes are listed in order of impact that this particular model uses in its formulation to predict churn.
  ...
  From the customer produt behavior point of view, it includes averages of transactions from the last Quarter (M3\_5) and the last 6 Months (M3\_8), as well as the number of transactions using different devices, types of payment and specific Genres of movies watched. 
  ...
  There were *No* customers that were *Predicted* to be *Non-Churners* when they were. 
  ...
  And the last cell the model *Predicted* 12 customers were going to *Churn* when they actually did, and the model was able to capture all *Churners* from this **Test Set**. 

4. Deploying the Model results and options for scoring

  Back in the **Leader Board** section, there are many actions that can be done at this level with a model.  Clicking on the *row of a model*, but not on the model name itself, would highlight that model in a blue hue.

  ![Churn AutoML Step 4 Leader Board selection](images/oml-churn-automl-leader-selection.png "Churn AutoML Step 4 Leader Board selection") 
  
  You will now be able to select any of the menus immediately above it, with the options to *Deploy*, *Rename*, *Create Notebook* and *Metrics*.

  We can first check out the **Metrics** menu option, by clicking on it.  A menu will pop open showing several metrics that can be selected.  Choose a few ones and just close the menu in the top right **X** when ready.

  ![Churn AutoML Step 4 Leader Board select metrics](images/oml-churn-automl-leader-metrics.png "Churn AutoML Step 4 Leader Board select metrics") 

  The new metrics will be displayed for each model and would allow you to compare and sort the list of models by each column desired.
  ...
  You have the option to **Deploy Model**, which would allow you to register the model with Oracle Machine Learning Services, and that would enable sub-second scoring from a REST interface that is suitable for real-time applications.

  An example of the Deployment menu is shown below, and it is optional for this **Workshop**.
  ...
  In the case of a successful Deployment, a notice at the top of the screen will show the following information.
  ...
  The next option available is the **Create Notebook** button, which would auto-generate an entire Python-based Notebook using Oracle Machine Learning for Python that would reproduce the creation of the Model from scratch, including all options identified by the AutoML process.

  Clicking on the **Create Notebook** button would open up a menu that would let you give it a name.  This step is also optional for this **Workshop**.
  ...
  On a successful **Notebook** creation, you would receive the following message.
  ...
  If the Rename is successful, you should see a message at the top of the screen.
  ...
  It will show up in the **Leader Board** with the new name as well.
...
5. Score the customers using the Random Forest model

  With the model renamed to a known name, you can now proceed to run a simple SQL query for creating a new table that will contain the list of customers with their **Probabilities to Churn**.
  ...
  **Click** <a href="./files/Scoring_customers_with_Churn_Model.json" download="Scoring_customers_with_Churn_Model.json" target="\_blank">here</a> to download the sample Scoring notebook for this lab, "Scoring\_customers\_with\_Churn\_Model.json", to a folder on your local computer.
  ...
  Back in the OML Notebooks screen, click the **Import** button and navigate to the folder where you downloaded the notebook **Scoring\_customers\_with\_Churn\_Model.json** file you just downloaded.
  ...
  In case of success, you should receive a notification at the top of the screen that the Import process worked, and you should be able to see a new notebook called **Scoring customers with Churn Model** in the list of Notebooks.
  ...
  We are going to open the Notebook for editing.  For that we need to **click on the "Scoring customers with Churn Model" name**.  You will see that the Notebook server starts and loads the Notebook, where you get to the top of the Notebook. 
  ...
  If we scroll down, we see bascically two main steps.  The first paragraph deletes a table named `POTENTIAL_CHURNERS` if it exists, and the second paragraph uses SQL to create a new table based on the PREDICTION and PREDICTION_PROBABILITY capabilities by OML.

  ![Churn AutoML Step 5 Scoring Notebook second screen](images/oml-churn-automl-notebook-screen2.png "hurn AutoML Step 5 Scoring Notebook second screen")

  Please note that the following code can be accessed by any Application through JDBC against this Autonomous Database instance, and the scoring will be generated.
  ...
  At the end, the following result is expected from the Scoring table created.

  ![Churn AutoML Step 5 Scoring Notebook third screen](images/oml-churn-automl-notebook-screen3.png "hurn AutoML Step 5 Scoring Notebook third screen")

  **CONGRATULATIONS!!!**

  You now have deployed a new table containing each customers likelihood to churn and the decision by the Model. 


## Learn More

* [Oracle Machine Learning product information](https://oracle.com/goto/machine-learning)
