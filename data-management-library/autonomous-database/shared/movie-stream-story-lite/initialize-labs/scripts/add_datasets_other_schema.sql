create or replace procedure add_datasets_other_schema (
    user_name varchar2 := 'moviestream'
)
as 

cmd clob;
proc_name varchar2(100) := 'add_datasets_exec';
add_datasets_name varchar2(200);

begin
add_datasets_name := user_name || '.' || proc_name;

cmd := 'create or replace procedure ' || add_datasets_name || ' as ' ||
q'[
...
*/
    ...
    -- Run the procedure
    moviestream_write(' - compiling ' || add_datasets_name);
    execute immediate 'alter procedure ' || add_datasets_name || ' compile';
    moviestream_write(' - run ' || add_datasets_name);
    
    execute immediate 'begin ' || add_datasets_name || '; end;';

end add_datasets_other_schema;