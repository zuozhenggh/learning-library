...
6.  Go to SmartView Ribbon –> Options –> Cell Styles –> Essbase –> Data Cells and pick colors for    Drill-through (blue), Read-Only (pink), Writable (green) and Linked Objects (yellow). Click OK.
...
## **Step 2:** Check the data in relational table(Optional)
   ...
   **Note**: The data was already loaded in the SQL developer.  
...
2. Enter Name "SalesBreakdown".   
   Click dropdown for Datasource and select the data source you created above called "SAMPLEDT.SalesTable". Select the data source column name on the left and provide the generation mapping on the right. Select the Report Columns check boxes for all the rows. 
