...
prompt
prompt begin
prompt  LBACSYS.SA_SYSDBA.CREATE_POLICY(
prompt     policy_name => 'OLS_CRM_DEMO'
prompt   , column_name => 'OLSLABEL'
prompt   , default_options => 'READ_CONTROL,WRITE_CONTROL,LABEL_DEFAULT,HIDE');
prompt end;;
prompt /
...
prompt
prompt begin
prompt LBACSYS.SA_SYSDBA.DISABLE_POLICY(
prompt policy_name => 'OLS_CRM_DEMO');
prompt end;;
prompt /
