...
select sa_session.read_label('OLS_CRM_DEMO') OLS_READ_LABEL, sa_session.write_label('OLS_CRM_DEMO') OLS_WRITE_LABEL , sa_session.label('OLS_CRM_DEMO') label from dual;

prompt 
prompt Change SA_SESSION label for CRM user
BEGIN
 SA_SESSION.SET_LABEL (
   policy_name => 'OLS_CRM_DEMO',
   label       => 'D:FGT:GBL,USA,EMEA,ENG,FRA,JPN');
END;
/
...
prompt
prompt select olslabel, label_to_char(olslabel) label_to_char, count(*) count_of_labels 
prompt  from crm.customers 
...
prompt
prompt select country, count(*) count_of_each, olslabel, label_to_char(olslabel) label_to_char
prompt  from crm.customers
