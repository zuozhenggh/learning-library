...
このハンズオンでは参加者のPCから演習環境（OCIインスタンス）に対してSSH接続を行った上、環境構築やアプリケーション開発のコマンドを発行します。そのため、事前にクライアントPC側でSSHキー・ペア（公開キー、秘密キー）を生成する必要があります。以下はクライアントPCの種類ごとにSSHキー・ペアーの生成手順です。

■ Windows 10

早期ビルドを除けば、Windows 10はSSHキー生成機能"ssh-keygen"を備えています。

1.  **Powershell** コマンドウィンドウを開きます。スタート->サーチフィールドに*powershell*を入力することで、"Windows Powershell"アプリケーションを検索できます。

    ![Open a Powershell.](images/keylab-005.png " ")

2.  ターミナルウィンドウに ```ssh-keygen``` を入力します。その際、コマンドが実行されるディレクトリーを確認しておいてください。通常SSHキーはこのディレクトリー*C:¥Users¥<ユーザ名>*の.ssh配下に生成されます。
...
4.  *C:¥Users¥<ユーザ名>*配下に以下のコマンドでSSHキー・ペアが正常に生成されることを確認します。
...
5. プロビジョニング済みのインスタンスの情報を確認し、パブリックIPアドレスは後の演習でSSH接続時必要になりますので、手元にメモをしておいてください。
