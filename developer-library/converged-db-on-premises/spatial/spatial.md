...
* Next we added Spatial metadata for the `CUSTOMERS`, `WAREHOUSES` and `WAREHOUSES_DTP` tables to the `USER_SDO_GEOM_METADATA` view. Each `SDO_GEOMETRY` column is registered with a row in `USER_SDO_GEOM_METADATA`.
