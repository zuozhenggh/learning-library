...
4. To limit the data entry for an item one of the simplest techniques is to convert a text item, which allows any data to be entered, into a select list.
    ...
    In Page Designer, within the Rendering tree (left pane), select **P5\_STATUS**. In the Property Editor (right pane), enter the following.
...
This completes Lab 3. You now know how _end-users_ can define custom reports using Interactive Reports, and you can easily improve form pages. [Click here to navigate to Lab 4](?lab=lab-4-link-calendar)
