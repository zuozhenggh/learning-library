...
### Task 1: Verify your Oracle Cloud Trial Account
...
### Task 3: Go to Compute Services and Create a Compartment
...
The region where your IAM resources reside. All IAM resources are global and available across all regions, but the master set of definitions reside in a single region, the home region. You must make changes to your IAM resources in your home region. The changes will be automatically propagated to all regions. For more information, see Managing Regions. In North America the Region will be Ashburn or Phoenix.
    ...
    An API key pair will be created by you later in this lab. You will upload that key into an OCI USER account so you will be able to remotely be able to create resources in your Tenancy. Once uploaded, a Fingerprint use by subsequent builds in Developer Cloud Service will be generated.
...
### Task 4: Create a Virtual Compute Network
...
- You'll notice for redundancy reasons that you have a subnet in each of the three Availability Domains in the region. Find the `<your-region>-AD-1` subnet and click **Copy**. Paste the Subnet OCID into your text editor.
...
### Task 5: Create the Developer Cloud Service Instance

Before you can use Oracle Developer Cloud Service to provision to Oracle Cloud Infrastructure you must deploy an DevCS instance (Development/Deployment environment), Then you will configure supporting connections which control VM builds and storage project artifacts.
...
### Task 7: Import a GitHub repository, Create Key Pairs
...
### Task 7a Mac/Linux: Create SSH Key Pairs (API and Instance) (Linux or Mac client)
...
### Task 7b Windows: Create SSH Key Pair Create SSH Key Pairs (API and Instance) (Windows client)
...
### Task 8: Copy Key pair contents into their respective files in the Git Repository
...
### Task 9: Configure the DevCS connection to your OCI account
...
### Task 10: Create Software Template and Virtual Machine configurations
...
### Task 11: Edit the Packer "build.json" file
...
### Task 14: Edit the Terraform configuration file
...
### Task 15: Build the Terraform job
...
### Task 18a: SSH into the `MYSQL` Compute Instance (Windows)
...
### Task 18b: SSH into the `MYSQL` Compute Instance (Linux/Mac)
...
### Task 19: Verify the MYSQL, Git and Docker installations

In this step you are SSH'ed into the `MYSQL` Compute instance and you will execute a few commands to verify that MYSQL was installed OK and the database schema for the AlphaOffice application is there.
