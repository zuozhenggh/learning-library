...
To establish proper access to a JDE Trial Edition, the Oracle Cloud Infrastructure (OCI) tenancy needs to be set up.
...
3.  Generate ssh-keys using the command `ssh-keygen` in Git Bash and then press “Enter” for all steps:
...
To create a VCN on OCI:

1.	On the Oracle Cloud Console home page, under **Quick Actions**, click **Set up a network with a wizard**.
...
In this tutorial, OCI has been set up for the networking required to be able to access a JDE Trial Edition that will be created in **Creating a Trial Edition Instance in OCI**.
